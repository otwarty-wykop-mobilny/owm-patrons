...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	fileContent, _, _, err := client.Repositories.GetContents(ctx, config.RepoAuthor, config.RepoName, config.TiersFilePath, &github.RepositoryContentGetOptions{
		Ref: config.Branch,
	})

	if err != nil {
		fmt.Fprintln(os.Stderr, "An error has occured while fetching tiers.json contents", err.Error())
		writeJSON(w, 500, J{
			"message": "An error has occured while fetching tiers.json contents",
			"error":   err.Error(),
		})
		return
	}
